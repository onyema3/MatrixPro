...
class Matrix_MLM_User_Genealogy {
    ...
    public function render($user_id) {
        ...
        if (class_exists('Matrix_MLM_User')) {
            ...
            // get_referral_link returns ".../signup/?ref=" with
     ...
     * "Genealogy hover-card" section. Originally was inline next to
     * this markup (same colocation rationale as render_level_badges()
     * and render_search_box()), but on real installs the body-level
     * <style> block was being silently stripped before reaching the
     * browser by:
     *   - HTML minifiers in caching plugins (LiteSpeed, W3 Total
     *     Cache, WP Rocket, Autoptimize) that combine-or-strip inline
     *     <style> tags as part of "Optimize CSS Delivery"
     *   - strict CSP headers (style-src 'self' without 'unsafe-inline')
     *   - shortcode output sanitisers that route through wp_kses_post()
     * Members reported the card always rendering in plain colours
     * even after clearing the browser cache, because the cached HTML
     * served by the host plugin had never contained the <style> in
     * the first place. Serving the rules from the registered
     * matrix-mlm-dashboard stylesheet sidesteps all three failure
     * modes (cache-busted by MATRIX_MLM_VERSION, <link> tag in <head>,
     * delivered as a same-origin CSS file).
     */
