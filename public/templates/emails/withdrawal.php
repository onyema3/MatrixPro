...
$status_color = strtolower($status) === 'approved' ? '#059669' : (strtolower($status) === 'pending' ? '#d97706' : '#dc2626');
$status_bg = strtolower($status) === 'approved' ? '#f0fdf4' : (strtolower($status) === 'pending' ? '#fffbeb' : '#fef2f2');
$status_border = strtolower($status) === 'approved' ? '#bbf7d0' : (strtolower($status) === 'pending' ? '#fde68a' : '#fecaca');

ob_start();
?>
<h2 style="color:#1f2937;font-size:20px;margin:0 0 16px;font-weight:600;"><?php _e('Withdrawal Update', 'matrix-mlm'); ?></h2>
...
<p style="color:#4b5563;font-size:15px;line-height:1.6;margin:0 0 24px;">
    <?php _e('Here is an update on your withdrawal request:', 'matrix-mlm'); ?>
</p>
...
<p style="color:#4b5563;font-size:14px;line-height:1.5;margin:0;">
    <?php _e('Your withdrawal was rejected and the full amount has been refunded to your Matrix wallet. Please contact support if you have questions.', 'matrix-mlm'); ?>
</p>
<?php endif; ?>
<?php
